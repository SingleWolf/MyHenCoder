...
public class ClipViewLayout extends RelativeLayout {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        mPreViewW = (int) (Math.min(mWidthPixels, mHeightPixels) - mHorizontalPadding * 2);
    }
    ...
    public void setScaleClipHW(float scaleClipHW) {
        if (mClipView != null) {
            mClipView.setScaleClipHW(scaleClipHW);
        }
    }
    ...
    public Bitmap clip() {
        ...
        try {
            ...
            // 对图片进行压缩
            zoomedCropBitmap = BitmapUtil.zoomBitmap(cropBitmap, mPreViewW, (int) (mPreViewW * mClipView.getScaleClipHW()));
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (cropBitmap != null) {
     ...
     * @return 指定路径
     */
    public String clip2Path(String filePath) {
        mImageView.setDrawingCacheEnabled(true);
        mImageView.buildDrawingCache();
        Rect rect = mClipView.getClipRect();
        Bitmap cropBitmap = null;
        try {
            cropBitmap = Bitmap.createBitmap(mImageView.getDrawingCache(), rect.left, rect.top, rect.width(), rect.height());
            FileUtil.saveBitmap2Path(cropBitmap, filePath);
        } catch (Exception e) {
            e.printStackTrace();
        }
        cropBitmap.recycle();
